...
def itertools_get_combos(turn_codes, trailing_YorZ_Xs):
    ...
    turn_COMP_codes = list(map(lambda x: (4-x)%4, turn_codes))


    positions_of_X_axis_turns = []
    for indx, turn_code in enumerate(turn_codes):
    ...
    for i in range(1, len(positions_of_X_axis_turns) + 1):
        combos_of_length_i = [list(x) for x in itertools.combinations(positions_of_X_axis_turns, i)]
        position_combos.extend(combos_of_length_i)
  

    trailing_X_rotation__target_num = CODE[trailing_YorZ_Xs[1]]
    combos_of_positions_whose_comps_sum_successfully:list[list[int]] = []
    for combo in position_combos:
        sum = 0
        for position in combo:
            sum = sum + turn_COMP_codes[position]   # Summing the COMPLEMENTS of different combinations of R, L turns
        if math.fmod(sum,4) == trailing_X_rotation__target_num:
            combos_of_positions_whose_comps_sum_successfully.append(combo)
    ...
    # Each sum_success is a just copy of turn_codes but with some elements changed to 0. The elements left untouched are those which, together, sum to the target value.
    DNA_1__sum_successes = []
    for successful_combo in combos_of_positions_whose_comps_sum_successfully:
        turn_codes___but_where_codes_with_badly_summing_comps_get_voted_off_the_island = []
        for i, turn_code in enumerate(turn_codes):
            if i not in successful_combo:
                turn_codes___but_where_codes_with_badly_summing_comps_get_voted_off_the_island.append(0)
            else: turn_codes___but_where_codes_with_badly_summing_comps_get_voted_off_the_island.append(turn_code)
        DNA_1__sum_successes.append(turn_codes___but_where_codes_with_badly_summing_comps_get_voted_off_the_island)


    # print("\nDNA_1_sum_successes  (a.k.a.  Every possible useful modification of turn_codes. Groups of elements that sum correctly with each other (mod 4) remain, and the rest are replaced by zeros):  ")
    # print("\n", DNA_1__sum_successes)
