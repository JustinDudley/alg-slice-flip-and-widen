...
def generate_algs(stic_turns, trailing_YorZ_Xs_dual, stic_alg):


    ALL_ripple_rounds__final_algs = []
    if is_test: ALL_ripple_rounds__final_algs = TESTING_1(ALL_ripple_rounds__final_algs, stic_alg, stic_turns)
    ...
    for trailing_YorZ_Xs in trailing_YorZ_Xs_dual:
        ...
        for indx in reversed(range(len(stic_turns) + 1)):   #   +1 because of the extra YorZ item. Don't want to use add_YorZ__shifting_listTypeAlg for this because it is changing.
            ...
            if indx == 0 or AXIS_FAMILY[YorZ_added__shifting_turns[indx - 1]] != AXIS_FAMILY[trailing_YorZ_Xs[0]]:
